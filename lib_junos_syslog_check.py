...
def get_syslog_messages(dut_host):
    ...
    command_set_1 = [f'show log messages | no-more | save /var/tmp/syslog_{current_time}',
    ...
    print(f'Logfile saved as /var/tmp/syslog_{current_time}.')
    return output


'''Parse output from logging data retrieved'''
def parse_system_messages(inputs):
    log_errors = []
    log_failures = []
    log_fail_no_route = []
    log_down = []
